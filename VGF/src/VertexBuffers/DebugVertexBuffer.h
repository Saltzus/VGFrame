...
struct DebugVertexBuffer : public VertexBuffer
{
    ...
    {
        // position attribute
        glVertexAttribPointer(lastLocation, 3, GL_FLOAT, GL_FALSE, sizeof(DebugVertex), (void*)offsetof(DebugVertex, position));
        glEnableVertexAttribArray(lastLocation);
        lastLocation++;

        // vertex color
        glVertexAttribPointer(lastLocation, 3, GL_FLOAT, GL_FALSE, sizeof(DebugVertex), (void*)offsetof(DebugVertex, color));
        glEnableVertexAttribArray(lastLocation);
        lastLocation++;
    }
};