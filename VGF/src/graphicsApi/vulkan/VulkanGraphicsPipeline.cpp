...
namespace VGF::Vulkan
{
    ...
    VkShaderModule createShaderModule(VkDevice device, const std::vector<char>& code) 
    {
        ...
        if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
            VGF::Log::Error("Failed to create shader module!");
        }
	...
	VulkanGraphicsPipeline::VulkanGraphicsPipeline(const PipelineConfig& config, VkDevice& device, VkDescriptorSetLayout& descriptorSetLayout, VkRenderPass& renderPass, VkPipelineLayout& pipelineLayout, VkPipeline& graphicsPipeline)
	{
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
            VGF::Log::Error("Failed to create pipeline layout!");
        }
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
            VGF::Log::Error("Failed to create graphics pipeline!");
        }
	...
	void VulkanGraphicsPipeline::compileErrors(unsigned int shader, const char* type)
	{
